...
class NexusModuleTestCase(TestCase):
    ...
    def test_update(self):
        ...
        resp = self.client.post(
            "/nexus/gargoyle/update/", {"curkey": "key1", "key": "key2"}
        )
        assert resp.status_code == 200
        body = json.loads(resp.content.decode("utf-8"))
        assert body["success"] is True
    ...
    def test_status(self):
        ...
        resp = self.client.post(
            "/nexus/gargoyle/status/", {"key": "key1", "status": str(GLOBAL)}
        )
        assert resp.status_code == 200
        body = json.loads(resp.content.decode("utf-8"))
        assert body["success"] is True
    ...
    def test_remove_condition(self):
        ...
        switch.add_condition(
            gargoyle, "gargoyle.builtins.IPAddressConditionSet", "ip_address", "1.1.1.1"
        )
        conditions = list(switch.get_active_conditions(gargoyle))
